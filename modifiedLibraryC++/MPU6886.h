...
#define MPU6886_GYROLP_CONFIG 0x1E
...
class MPU6886 {
 public:
  ...
  Gscale Gyscale = GFS_500DPS;//default:GFS_2000DPS
  Ascale Acscale = AFS_4G;//default:AFS_8G;
 ...
 public:
  ...
  void getGyroOffAdc(int16_t* gxoff, int16_t* gyoff, int16_t* gzoff);
  void getGyroOffset(float* gx, float* gy, float* gz);

  void SetGyroFsr(Gscale scale);
  void SetAccelFsr(Ascale scale);

  void getAhrsData(float* pitch, float* roll, float* yaw);
  void getAhrsData2(float* pitch, float* roll, float* yaw,float gyroX, float gyroY, float gyroZ,float accX, float accY, float accZ);
 public:
  float aRes, gRes;
