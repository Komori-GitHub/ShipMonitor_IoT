...
int MPU6886::Init(void) {
  ...
  regdata =  0x06;//default:0x00
  I2C_Write_NBytes(MPU6886_ADDRESS, MPU6886_ACCEL_CONFIG2, 1, &regdata);
  delay(1);

  regdata =  0xD0;//Low Power mode on
  I2C_Write_NBytes(MPU6886_ADDRESS, MPU6886_GYROLP_CONFIG, 1, &regdata);
  delay(1);
...
void MPU6886::getAhrsData2(float* pitch, float* roll, float* yaw,float gyroX, float gyroY, float gyroZ,float accX, float accY, float accZ) {
